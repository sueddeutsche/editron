...
describe("utils/createDiff", () => {
    ...
    it("should return nested patches with pointer of change and its patch", () => {
        const result = createDiff({ a: { changedHere: "prop" } }, { a: { changedHere: "properly" } });
        assert.equal(result.length, 1);
        assert.deepEqual(result[0], {
    ...
    it("should return all patches with pointer of change and its patch", () => {
        const result = createDiff({ a: { changedHere: "prop" }, andHere: "boo" }, { a: { changedHere: "properly" }, andHere: "foo" });
    ...
    it("should return pointer of object for key changes", () => {
        const result = createDiff({ a: { b: "key" } }, { a: { c: "key" } });
        assert.equal(result.length, 1);
        assert.deepEqual(result[0], {
            pointer: "#/a",
    ...
    it("should return pointer of array for item changes", () => {
        const result = createDiff({ a: ["string"] }, { a: ["modifiedString"] });
        assert.equal(result.length, 1);
        assert.deepEqual(result[0], {
            pointer: "#/a",
    ...
    it("should return movement of arrays", () => {
        const result = createDiff({ a: [{ _id: 0, title: "first" }, { _id: 1, title: "second" }, { _id: 2, title: "third" }] }, { a: [{ _id: 2, title: "third" }, { _id: 1, title: "second" }, { _id: 0, title: "first" }] });
        assert.equal(result.length, 1);
    });
    it("should return patches in correct order", () => {
        const result = createDiff({ a: [{ _id: 0, title: "first" }, { _id: 1, title: "second" }, { _id: 2, title: "third" }] }, { a: [{ _id: 2, title: "third" }, { _id: 1, title: "zwei" }, { _id: 0, title: "eins" }] });
