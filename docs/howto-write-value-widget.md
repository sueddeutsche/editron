...
> There is a difference, between an editor for arrays and object and more simple editors for values, like string, boolean, number. Object- and array-editors have to manage child-editors (or multiple values). This documentation refers to custom editors for _simple values_ only, but the concepts are the same.
...
```ts
import { AbstractValueEditor } from "editron";


class MyCustomEditor extends AbstractValueEditor {
...
const editron = new Controller(schema, { title: "test", teaser: "test of a custom editor" }, {
